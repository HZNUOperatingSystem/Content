...
- [ ] `kernel/sched/stride.c`：补全 `sched_init_proc()`、`stride_for_tickets()`、`sched_set_tickets()`、`sched_scan_best()`、`sched_commit()`、`sched_set_priority()`。
- [ ] `kernel/syscall.h` + `kernel/syscall.c` + `kernel/sysproc.c` + `user/usys.pl` + `user/user.h`：注册 `setpriority` 和 `getpinfo` 系统调用。
...
用户程序调用 `setpriority(50)` 时，数据是这样流动的：

```
  user code               usys.pl 生成的 stub         syscall.c 分发表
  setpriority(50)  →  li a7, SYS_setpriority; ecall  →  syscalls[23]
                            ↓
                    sysproc.c 处理函数               stride.c 真正逻辑
                    sys_setpriority()           →  sched_set_priority()
```
...
| 文件 | 做什么 |
|:---|:---|
| `kernel/syscall.h` | 分配一个系统调用号（`#define SYS_xxx N`） |
| `kernel/syscall.c` | 声明处理函数 + 填入分发表 `syscalls[]` |
| `kernel/sysproc.c` | 实现处理函数：用 `argint/argaddr` 取参数 → 调内核函数 |
| `user/usys.pl` | 加 `entry("xxx")`，perl 脚本自动生成汇编 stub |
| `user/user.h` | 声明用户态函数签名，用户程序才能调用 |

你将在 `setpriority` 和 `getpinfo` 两个系统调用上各走一遍这个流程。
...
`sched_pick_stride()` 要扫描 `proc[]`，找到 `RUNNABLE` 且 `pass` 最小的进程。
...
返回时要保持 `best->lock` 已经持有，和 RR 一样交给 `scheduler()` 在切回来后释放。
