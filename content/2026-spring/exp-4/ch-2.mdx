...
- [ ] `kernel/sched/stride.c` + `kernel/sched/sched.h`：补全 `sched_init_proc()`、`stride_for_tickets()`、`sched_set_tickets()`、`sched_scan_best()`、`sched_commit()`、`sched_set_priority()`，并在头文件中添加新函数的声明。
